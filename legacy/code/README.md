...
**This legacy code is preserved for golden dataset population in Arize Phoenix.** The main codebase (in the project root) now uses Qdrant for vector storage and MCP integration, but this legacy implementation provides:

- **RAGAS golden test dataset generation** (`langchain_eval_golden_testset.py`)
- **PostgreSQL pgvector data** that can be migrated to populate Qdrant collections
- **Phoenix experiment framework** with structured evaluation metrics
- **Proven evaluation infrastructure** for RAG system benchmarking

Use this code to generate golden datasets via RAGAS, then migrate the data to the current Qdrant-based implementation for enhanced evaluation capabilities.
